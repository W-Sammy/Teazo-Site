...
export async function PUT(
  ...
  try {
    ...
    const currentVariation = currentItem.itemData?.variations?.[0] as
    ...
    const upsertResult = await squareClient.catalog.object.upsert({
      ...
      object: {
        ...
        itemData: {
          ...
          modifierListInfo:
            body.modifierListIds !== undefined
              ? body.modifierListIds.map((mid) => ({
                  modifierListId: mid,
                  enabled: true,
                }))
          ...
          variations: [
            {
              ...
              itemVariationData: {
                ...
                priceMoney: {
                  amount: BigInt(
                    body.priceCents ?? Number(currentPrice?.amount ?? 0),
                  ),
    ...
    if (!menuItem) {
      return Response.json(
        { error: "Failed to build updated item" },
        { status: 500 },
      );
    }
