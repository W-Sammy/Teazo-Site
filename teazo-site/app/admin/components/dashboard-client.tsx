...
import { HoursCard, LiveEvents, SummaryCard, VisitChart } from "./dashboard";
...
export default function DashboardClient({ metrics, events, websiteContent }: DashboardClientProps) {
  ...
  return <div className="min-h-full bg-white px-5 pb-14 pt-8 text-[#273142] sm:px-8 lg:px-12 lg:pt-10"><div className="mx-auto max-w-6xl"><header className="mb-10"><p className="mb-2 text-xs font-semibold uppercase tracking-[0.18em] text-[#dbb082]">Overview</p><h1 className="text-3xl font-bold tracking-tight text-[#2b211d]">Dashboard</h1><p className="mt-2 text-sm text-slate-500">See how guests are engaging with your restaurant.</p></header><section aria-label="Visit summary" className="grid gap-4 sm:grid-cols-3"><SummaryCard label="Total visits (Top 5)" value={totalVisits.toLocaleString()} /><SummaryCard label="Most visited item" value={topItem?.item_name_snapshot ?? "—"} /><SummaryCard label="Top item visits" value={topItem?.event_count.toLocaleString() ?? "—"} /></section><VisitChart items={topItems} period={period} onPeriodChange={setPeriod} /><div className="mt-8 grid gap-6 lg:grid-cols-[1fr_1.35fr]"><HoursCard content={websiteContent} hours={getTodayHours(websiteContent)} todayHoliday={getTodayHoliday(websiteContent)} /><LiveEvents events={activeEvents} /></div></div></div>;
}
