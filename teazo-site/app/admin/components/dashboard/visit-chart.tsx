...
export default function VisitChart({ items, period, onPeriodChange }: VisitChartProps) {
  ...
  return <section className="mt-10 rounded-2xl bg-white p-5 sm:p-7" aria-labelledby="top-items-heading">
    ...
    <div className="space-y-4">{items.map((item) => <div key={item.square_item_id} className="grid grid-cols-[96px_minmax(0,1fr)_auto] items-center gap-3 text-sm sm:grid-cols-[120px_minmax(0,1fr)_auto] sm:gap-5"><span className="truncate text-slate-500">{item.item_name_snapshot}</span><div className="h-8 overflow-hidden rounded-lg bg-[#fcf5ed]"><div className="flex h-full items-center justify-end rounded-lg bg-[#dfb282] px-2 text-xs font-semibold text-white transition-all duration-300" style={{ width: `${Math.max((item.event_count / maxVisits) * 100, 15)}%` }}><span className="hidden sm:inline">{item.event_count.toLocaleString("en-US")} visits</span></div></div><span className="text-xs font-medium text-slate-400 sm:hidden">{item.event_count.toLocaleString("en-US")}</span></div>)}</div>
