...
export default function GeneralButton({ text, href }: Editables) {
    return (
        <Link 
            href={href} 
            className="block w-[213px]"
            target="_blank"
            rel="noopener noreferrer"
        >
            <button className={`${montserrat.className} w-[213px] h-[70px] bg-black text-white text-[18px] flex items-center justify-center hover:bg-[#FFBDC7] transition-colors tracking-[0.05em] uppercase`}>
                {text}
            </button>
        </Link>
    );
